...
void setup() {
  ...
  Serial.println("\n\n====================================");
  Serial.println("Portenta H7 USB HID Keyboard via MQTT");
  Serial.println("====================================\n");
...
void pressKeyCombo(const char* combo) {
  ...
  // Build modifier byte
  if (comboStr.indexOf("CTRL") >= 0) modifiers |= 0x01;  // Left Ctrl
  if (comboStr.indexOf("SHIFT") >= 0) modifiers |= 0x02;  // Left Shift
  if (comboStr.indexOf("ALT") >= 0) modifiers |= 0x04;  // Left Alt
  if (comboStr.indexOf("GUI") >= 0 || comboStr.indexOf("WIN") >= 0) {
    modifiers |= 0x08;  // Left GUI/Win
  }
  ...
  uint8_t keyCode = 0;
  if (comboStr.indexOf("DELETE") >= 0) keyCode = KEY_DELETE;
  else if (comboStr.indexOf("ENTER") >= 0) keyCode = KEY_RETURN;
  else if (comboStr.indexOf("TAB") >= 0) keyCode = KEY_TAB;
  else if (comboStr.indexOf("ESC") >= 0) keyCode = KEY_ESC;
  else if (comboStr.indexOf("BACKSPACE") >= 0) keyCode = KEY_BACKSPACE;
  else if (comboStr.indexOf("SPACE") >= 0) keyCode = ' ';
...
uint8_t getKeyCode(const char* keyName) {
  ...
  // Special keys mapping (USB HID keycodes)
